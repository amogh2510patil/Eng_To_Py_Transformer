...
def get_all_sentences(ds, section):
    for item in ds:
        if item[section]:
            yield item[section]

def get_or_build_tokenizer(config, ds, section):
    tokenizer_path = Path(config['tokenizer_file'].format(section))
    if not Path.exists(tokenizer_path):
        ...
        if section=='question':
            ...
            tokenizer.train_from_iterator(get_all_sentences(ds, section), trainer=trainer)
...
def dataset_load():
    ...
    ds_raw=parse_data(file_lines)

    ds_augemented = augement_dataset(ds_raw)
    return ds_augemented  
...
def get_ds(config):
    ...
    tokenizer_tgt = get_or_build_tokenizer(config, ds_raw, 'solution')
    ...
    train_ds = BilingualDataset(train_ds_raw, tokenizer_src, tokenizer_tgt, 'question', 'solution', config['seq_len'])
    val_ds = BilingualDataset(val_ds_raw, tokenizer_src, tokenizer_tgt, 'question', 'solution', config['seq_len'])
